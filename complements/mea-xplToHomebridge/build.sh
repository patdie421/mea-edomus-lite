...
cp "$ORG"/xplDeviceMsgLogger.js "$ORG"/xplHttpBridge.js "$ORG"/xplToHomebridge.js "$DIR"
cp "$ORG"/xplToHomebridge.json "$DIR"/config

_DIR=`escape "$DIR"`
REGEX1="s/###SERVICE_HOMEDIR###/$_DIR/g"

PROG=xplToHomebridge.js
PROG_OPTIONS="--cfgFile config/xplToHomebridge.json -p 8081"
_PROG_OPTIONS=`escape "$PROG_OPTIONS"`
REGEX2="s/###SERVICE###/$PROG/g"
REGEX3="s/###SERVICE_OPTIONS###/$_PROG_OPTIONS/g"
cat "$ORG"/scripts/_ctrl.template | sed -e "$REGEX1" -e "$REGEX2" -e "$REGEX3" > "ctrl_$PROG.sh"
chmod +x "ctrl_$PROG.sh"
