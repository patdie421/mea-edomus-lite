...
{
   if(lib) {
      interfacesFns->malloc_and_init2 = (malloc_and_init2_f)dlsym(lib, "malloc_and_init2_interface_type_006");
      if(!interfacesFns->malloc_and_init2) {
         fprintf(stderr,"malloc_and_init2: %s\n", dlerror());
      }
      interfacesFns->get_monitoring_id = (get_monitoring_id_f)dlsym(lib, "get_monitoring_id_interface_type_006");
      if(!interfacesFns->get_monitoring_id) {
         fprintf(stderr,"get_monitoring_id: %s\n", dlerror());
      }
      interfacesFns->get_xPLCallback = (get_xPLCallback_f)dlsym(lib, "get_xPLCallback_interface_type_006");
      ...
      interfacesFns->set_xPLCallback = (set_xPLCallback_f)dlsym(lib, "set_xPLCallback_interface_type_006");
      if(!interfacesFns->set_xPLCallback) {
         fprintf(stderr,"set_xPLCallback: %s\n", dlerror());
      }
      interfacesFns->get_type = (get_type_f)dlsym(lib, "get_type_interface_type_006");
