...
//               DEBUG_SECTION mea_log_printf("%s (%s) : Nb elements in queue after TIMEOUT : %ld)\n", DEBUG_STR, __func__, params->queue->nb_elem);
      ...
      if(!ret) {
         ...
         cJSON *jsonInterface = findInterfaceByDevNameThroughIndex_alloc(NULL, interfaceName);
         if(jsonInterface) {
            cJSON *jsonDevices = cJSON_GetObjectItem(jsonInterfaces, "devices");
            if(jsonDevices) {
...
interface_type_003_t *malloc_and_init_interface_type_003(int driver_id, cJSON *jsonInterface);
{
   interface_type_003_t *i003;
