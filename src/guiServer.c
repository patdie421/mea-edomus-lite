...
mea_error_t start_httpServer(uint16_t port, char *home)
{
   if(port==0)
      port=8083;
...
int start_guiServer(int my_id, void *data, char *errmsg, int l_errmsg)
{
   ...
   if(appParameters_get("GUIPATH", httpServerData->params_list) &&
      appParameters_get("GUIPORT", httpServerData->params_list)) {

      _httpServer_monitoring_id=my_id;
      err=start_httpServer(atoi(appParameters_get("GUIPORT", httpServerData->params_list)), appParameters_get("GUIPATH", httpServerData->params_list));
      if(err==NOERROR) { 
