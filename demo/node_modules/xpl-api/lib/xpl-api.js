...
class XplAPI extends Events.EventEmitter {
  constructor(configuration) {
    ...
    var debugOn = this._configuration.debugOn || false;
    if (debugOn)
       debug.enabled=true
    ...
    this._configuration.networkInterface = configuration.networkInterface || false;
    this._configuration.localAddress = false;
    ...
    for (var name in nis) {
      ...
      for (var i = 0; i < addrs.length; i++) {
        ...
        if ( this._configuration.networkInterface == false || 
             this._configuration.networkInterface == name ) {
          this._configuration.localAddress = addr.address;
          this._configuration.localNetmask = addr.netmask
          break;
        }
      }
    ...
    if(!this._configuration.localNetmask) {
      throw new Error('can not determine broadcast address')
    }

    var ba=[];
    var addr=(this._configuration.localAddress).split(".");
    var mask=(this._configuration.localNetmask).split(".");
    for(var i=0;i<4;i++) {
      ba[i]=parseInt(addr[i],0) | (255 - parseInt(mask[i],0));
    }
    this._configuration.broadcastAddress = ba[0]+"."+ba[1]+"."+ba[2]+"."+ba[3];
